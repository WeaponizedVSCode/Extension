...
`Foam()` 静态方法有三个代码路径。"扩展未激活 → 激活它"的路径缺少 try/catch。如果 `foamExtension.activate()` 被拒绝，将成为未处理的 Promise 拒绝。

**修复：** 将激活 + 导出读取序列包装在单个 try/catch 中，失败时返回 `undefined`。
