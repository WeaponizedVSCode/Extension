...
def main():
    ...
    template_root = os.path.join(repo_root, "resources", "setup-template")
    output_ts = os.path.join(repo_root, "src", "features", "setup", "assets.ts")
    ...
    with open(output_ts, "w", encoding="utf-8") as output_file:
        ...
        file_print("// This file is auto-generated by scripts/gen-setup.py")
        file_print("// Do not edit this file directly")
        file_print("// Run `python3 scripts/gen-setup.py` to regenerate this file")
        file_print("export let fs = {")
        for filename in sorted(value_map.keys()):
            file_print(f'\t"{filename}": atob("{value_map[filename]}"),')
        file_print("};")
